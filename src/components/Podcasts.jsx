...
const Podcasts = () => {
  ...
  const processedPodcasts = useMemo(() => {
    ...
    if (searchQuery.trim() !== '') {//check if search query is not empty
      result = result.filter((p) =>//filter podcasts based on search query
    ...
    if (selectedGenre) {//filter if a genre is selected
        result = result.filter((p) => {//filter podcasts based on selected genre
    ...
    result.sort((a, b) => {//sort podcasts based on sortBy state
      if (sortBy === 'date-desc') return new Date(b.lastUpdated) - new Date(a.lastUpdated);//newest first
  ...
  useEffect(() => {//reset current page to 1 when filters change
